...
exports.setup = function (assets, options) {
  ...
  return function (req, resp, next) {
    ...
    if (m && m.index === 0) {
      true_path = req.url.slice(prefix.length + 33); // 32 character md5 + 1 for slash
      exists = false;
    ...
    resp.on('header', function () {
      if (resp.getHeader('ETag')) resp.removeHeader('ETag');
      if (resp.getHeader('Last-Modified')) resp.removeHeader('Last-Modified');
    });
    next();
  };
};
