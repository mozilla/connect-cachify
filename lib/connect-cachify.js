...
/**
 * production - boolean indiciating if your in development
 *              or production mode. Effects how links for
 *              js and css files are generated.
 * 
 * root - Array of fully qualifid paths. Each path is a root
 *        where static resources are served up. This is the
 *        same value you'd send to the static middlware.
 * 
 * js - Associative Array of JavaScript files. The keys are
 *      a filename of your minified, optimized file. The values
 *      are lists of files that are used to build the minified
 *      production version. Cachify is build tool agnostic, 
 *      but should work well with RequireJS, Uglify, jsmin or
 *      whatever ungodly build script you have. Keys should
 *      match what you use with cachify_js.
  ...
  if (opts['production'] !== true &&
      ...
      opts[family][filename]) {
        return und.map(opts[family][filename], function (f) {
          var new_filename = hashify(f);
          return format(link_fmt, new_filename);
        }).join('\n');
  }
